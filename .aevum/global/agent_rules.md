...
26. **Impact Decision Gate (Cổng đánh giá ảnh hưởng & Chống Rác)**: Mọi vòng đời Task/Plan BẮT BUỘC phải tự động phân loại mức độ và chèn cờ đồng bộ vào cuối Checklist:
    - **Minor Task** (Fix bug nhỏ, CSS, Typo, Code Refactor nội bộ): BẮT BUỘC chốt bằng `[x] Bypass Sync: Đã đánh giá đây là Minor Task, không yêu cầu đồng bộ cấu trúc, không đẻ thêm thẻ Plan/Feature rác.`
    - **Major Task** (Thêm file, thay đổi luồng, đổi API, thư viện mới): BẮT BUỘC chốt bằng `[ ] Structural Sync: Đồng bộ toàn bộ Index, Feature, Plan tương ứng.`
